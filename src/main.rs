...
const DIFF: f32 = 5e-9;
const APPROX: u32 = 20;
const STIR: f32 = 5_000.0;

fn idx((x, y): (i32,i32)) -> usize {
    (N  * y.rem_euclid(N ) + x.rem_euclid(N )) as usize
}

fn coord(i: usize) -> (i32, i32) {
    (i as i32 % N, i as i32 / N)
}
...
fn grid_corner((x, y): (i32, i32)) -> (i32, i32) {
...
impl Engine {
    fn init() -> Engine {
        ...
        Engine {
            ...
            running: true,
            time: Instant::now(),
            dye: vec![0.0; (N * N) as usize],
            vel: vec![(0.0,0.0); (N * N) as usize],
            draw_mode: 0
        }
    }

    fn diffuse_dye(&mut self, dt: f32) { // may be wrong
        let rate = dt * DIFF * (N as f32 * N as f32); // why?
        for _k in 0..APPROX { // is 20 in paper
            let mut updated_dye_field = vec![0.0; (N * N) as usize]; // TODO DONT REALLOC
            for y in 0..N {
                for x in 0..N {
    ...
    fn advect_dye(&mut self, dt: f32) {
        let mut updated_dye_field = vec![0.0; (N * N) as usize];
        for y in 0..N {
            for x in 0..N {
                ...
                updated_dye_field[idx((x ,y))] = blerp(v0, v1, v2, v3, frac_x, frac_y);
            }
        }
        self.dye = updated_dye_field;
    }
    ...
    fn diffuse_velocities(&mut self, dt: f32) {
        let rate = dt * VISC * (N as f32 * N as f32);
        for _k in 0..APPROX {
            let mut updated_vel_field = vec![(0.0,0.0); (N * N) as usize];
            for y in 0..N {
                for x in 0..N {
                    let i = idx((x,y));
    ...
    fn remove_divergence(&mut self) {
        let mut p = vec![0.0; (N * N) as usize];
        let h = 1.0 / N as f32; // why size?

        // find divergence
        let mut div = vec![0.0; (N * N) as usize];
        ...
        for i in 0..(N * N) as usize {
            ...
            self.vel[i].0 -= 0.5 * (e + w) / N as f32; // why size?
            self.vel[i].1 -= 0.5 * (n + s) / N as f32; // 
        }
    }
    ...
    fn render(&mut self) {
        ...
        for (i, (dye, (vx, vy))) in self.dye.iter().zip(self.vel.iter()).enumerate() {
            let (x, y) = grid_corner(coord(i));  
            if  self.draw_mode == 1 && *dye > 0.0 {
                self.canvas.set_draw_color(Color::RGB(0, (dye.powf(0.5) * 255.0) as u8, 0));
                self.canvas.fill_rect(Rect::from((x, y, RATIO as u32, RATIO as u32))).unwrap();
            }
            if self.draw_mode == 0 {
